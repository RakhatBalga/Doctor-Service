...
// NewDoctorUseCase wires the use-case with its dependencies.
func NewDoctorUseCase(repo repository.DoctorRepository, cacheRepo cache.CacheRepository, cacheTTL time.Duration, publisher event.EventPublisher, logger *slog.Logger) *DoctorUseCase {
	if publisher == nil {
	...
	return &DoctorUseCase{repo: repo, cacheRepo: cacheRepo, cacheTTL: cacheTTL, publisher: publisher, logger: logger, now: time.Now}
}
...
func (uc *DoctorUseCase) CreateDoctor(ctx context.Context, in CreateDoctorInput) (*model.Doctor, error) {
	...
	// Write-Through: invalidate list key
	_ = uc.cacheRepo.Delete(ctx, "doctors:list")
...
func (uc *DoctorUseCase) GetDoctor(ctx context.Context, id string) (*model.Doctor, error) {
	...
	cacheKey := "doctor:" + id
	var d model.Doctor
	err := uc.cacheRepo.Get(ctx, cacheKey, &d)
	if err == nil {
		return &d, nil // Cache hit
	}
	...
	// Set cache
	_ = uc.cacheRepo.Set(ctx, cacheKey, doc, uc.cacheTTL)
...
func (uc *DoctorUseCase) ListDoctors(ctx context.Context) ([]*model.Doctor, error) {
	...
	err := uc.cacheRepo.Get(ctx, cacheKey, &doctors)
	if err == nil {
		return doctors, nil // Cache hit
	}
	
	doctors, err = uc.repo.List(ctx)
	if err != nil {
		return nil, err
	}
	
	// Set cache
	_ = uc.cacheRepo.Set(ctx, cacheKey, doctors, uc.cacheTTL)
